...
This tutorial will walk you through 7 steps:

::: {.callout-note title="PIPELINE STEPS"}
| Step | Purpose | Main outputs |
|-------|---------|--------------|
| 1 | Import paired-end reads, assess quality using **FASTQC** | `.qza` (raw sequences), `.qzv` (quality summaries) |
| 2 | Remove primers, adapters | `.qza` (trimmed reads), `.qzv` (post-trim quality) |
| 3 | Independent quality control (optional) | FASTQC reports |
| 4 | Determine truncation lengths using **Figaro** | Truncation parameters (`.json`) |
| 5 | Denoise, merge reads using **DADA2** | ASV table, representative sequences, denoising stats |
| 6 | Assign taxonomy using **SILVA** classifier | Taxonomy `.qza`, `.qzv` |
| 7 | Export files for **R / phyloseq** | `.tsv`, `.txt` files |
:::
...
> Stop here! **You will need to pre-trained Classifier in QIIME2 before moving forward.** Refer to this tutorial to pre-train a classifier... [QIIME2 Classifer](qiime/scripts/dbTrainer.qmd){.btn .btn-primary .me-2}
